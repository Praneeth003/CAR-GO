...
export class DeleteVariantComponent implements OnInit {
  ...
  async getVariants(){
    ...
    // let copySyDetailsFilterelectedFIlterParams = this.utils.deepClone(this.selectedFilterParams);
    // let copyJourneyDetails = this.utils.deepClone(this.journeyDetailsFilter);
    let endPoint = 'variant_s';
    let params = this.selectedFilterParams;
  ...
  async deleteCar(item){
    console.log(item);
    let endPoint = 'delete_variant';
    let confirmResponse = confirm("Confirm to delete the car " + item.variantName);
    if(confirmResponse){
      let result = await this.waterDataService.delete(endPoint + "/" + item.variantId).toPromise()
      if(result != null && result != undefined){
        if(result['status'] == this.constantsModule.HTTP_STATUS.SUCCESS){
          this.toastrService.success(item.variantName + " deleted succesfully")
          this.getVariants();
          return;
        }
      }
      this.toastrService.error("Unable to delete the car " + item.variantName + ", Please Contact Customer Care... ")
    }
